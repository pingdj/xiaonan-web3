...
class MarketMixin:
    ...
    def market(self, pair: str) -> dict:
        ...
        if not result.get("success"):
            raise Exception(f"市场数据获取失败：{result.get('error', '未知错误')}")
        return result
    ...
    def risk(self, pair: str, side: str, leverage: float = 10,
        ...
        result = resp.json()
        if not result.get("success"):
            raise Exception(f"风控检查失败：{result.get('error', '未知错误')}")
        return result
    ...
    def token_check(self, address: str, chain_id: str = "56") -> dict:
        ...
        result = resp.json()
        if not result.get("success"):
            raise Exception(f"代币安全检测失败：{result.get('error', '未知错误')}")
        return result
    ...
    def token_audit(self, contract_address: str, chain_id: str = "56") -> dict:
        ...
        result = resp.json()
        if not result.get("success"):
            raise Exception(f"代币审计失败：{result.get('error', '未知错误')}")
        return result
    ...
    def dex(self, token_address: str, chain: str = "bsc") -> dict:
        ...
        result = resp.json()
        if not result.get("success"):
            raise Exception(f"DEX分析失败：{result.get('error', '未知错误')}")
        return result
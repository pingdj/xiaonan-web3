...
class MediaMixin:
    ...
    def video_parse(self, url: str) -> dict:
        ...
        if not result.get("success"):
            raise XiaoNanError("视频解析失败", original_error=result.get('error', '未知错误'))
        return result
    ...
    def digital_human(self, text: str, image_url: str,
        ...
        result = resp.json()
        if not result.get("success"):
            raise XiaoNanError("数字人任务提交失败", original_error=result.get('error', '未知错误'))
    ...
    def digital_human_v2(self, text: str, image_url: str,
        ...
        result = resp.json()
        if not result.get("success"):
            raise XiaoNanError("数字人V2任务提交失败", original_error=result.get('error', '未知错误'))

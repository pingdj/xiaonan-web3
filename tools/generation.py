...
class GenerationMixin:
    ...
    def tts(self, text: str, voice: str = "BV007_streaming") -> str:
        ...
        if not result.get("success"):
            raise XiaoNanError("TTS 失败", original_error=result.get('error', '未知错误'))
    ...
    def podcast(self,
        ...
        result = resp.json()
        if not result.get("success"):
            raise XiaoNanError("播客任务提交失败", original_error=result.get('error', '未知错误'))
    ...
    def article(self, keywords: str,
        ...
        result = resp.json()
        if not result.get("success"):
            raise XiaoNanError("AI 创作失败", original_error=result.get('error', '未知错误'))
        return {
    ...
    def wait(self, interval: int = 10, timeout: int = 120) -> str:
        ...
        while True:
            ...
            if result.get("status") == "error":
                raise XiaoNanError("播客生成失败", original_error=result.get('error_msg', '未知错误'))
            time.sleep(interval)
...
class DataMixin:
    ...
    def extract(self, url: str, mode: str = "basic") -> dict:
        ...
        if not result.get("success"):
            raise XiaoNanError("网页提取失败", original_error=result.get('error', '未知错误'))
        return result
    ...
    def newsflash(self, category: str = "all", keyword: str = "",
        ...
        result = resp.json()
        if not result.get("success"):
            raise XiaoNanError("快讯获取失败", original_error=result.get('error', '未知错误'))
        return result
    ...
    def trump_feed(self, action: str = "get") -> dict:
        ...
        result = resp.json()
        if not result.get("success"):
            raise XiaoNanError("特朗普动态获取失败", original_error=result.get('error', '未知错误'))
        return result